...
const ThreeDCard = ({ children, className, glareOpacity = 0.2, tiltDegree = 10, perspective = 1000, glareSize = 100, borderRadius = 8, scale = 1.02, disabled = false }: ThreeDCardProps) => {
  ...
  // Throttle mouse move for better performance - increased to 32ms (approx 30fps) for better performance
  ...
  return (
    <div
      ...
      style={{
        ...
        transform: isHovered && !effectiveDisabled ? `rotateX(${rotation.x}deg) rotateY(${rotation.y}deg) scale(${effectiveScale})` : "rotateX(0) rotateY(0) scale(1)",
